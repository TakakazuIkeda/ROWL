...
public class CharacterControl : MonoBehaviour
{
    ...
    {
        /// playablecharacterIsAGirlを、GirlCharacterのThirdPersonCharacterが機能しているときと定義する
        var playableCharacterIsAGirl = GirlCharacter.GetComponent<ThirdPersonUserControl>().enabled;
        /// 女性キャラクターを操作しているとき、カメラが男性キャラクターを追従する
        if (playableCharacterIsAGirl == true)
        {
            autoCam.SetTarget(BoyCharacter.transform);
        }
        /// それ以外、つまり男性キャラクターを操作しているとき、カメラが女性キャラクターを追従する
        else
        {
            autoCam.SetTarget(GirlCharacter.transform);
        }
        /// ThirdPersonUserControlとAICharacterControlとが切り替えられ、同時に機能しないように
        /// girlとboyとの操作が切り替えられ、同時に操作可能にならないように
